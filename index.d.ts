...
interface GameMp {
   ...
   * @param hash название функции
   * @param args аргументы
   * @return возвращаемое значение функции.
   */
  invoke(hash: string, ...args: any[]): any;
  /**
   * Вызывает нативную функцию, которая возвращает вещественное число.
   * @param hash название функции
   * @param args аргументы
   * @return результат выполнения функции в виде вещественного числа
   */
  invokeFloat(hash: string, ...args: any[]): any;
  /**
   * Вызывает нативную функцию, которая возвращает строку.
   * @param hash название функции
   * @param args аргументы
   * @return результат выполнения функции в виде строки.
   */
  invokeString(hash: string, ...args: any[]): any;
  /**
   * Вызывает нативную функцию, которая возвращает вектор.
   * @param hash название функции
   * @param args аргументы
   * @return результат выполнения функции в виде вектора.
   */
  ...
  /**
   * Генерирует массив хэшей на основе массива строк.
   * @param textArray массив строк
   * @return массив хэш-строк.
   */
  joaat(textArray: string[]): Hash[];
  /**
   * Приостанавливает выполнение текущего скрипта.
   * @param ms время остановки в миллисекундах
   */
  wait(ms: number): void;
  /**
   * Асинхронно приостанавливает выполнение текущего скрипта (не блокируя выполнение основного потока).
   * @param ms время остановки в миллисекундах
   */
...
interface GuiMp {
  ...
  /**
   * Выполнить JavaScript код внутри браузера.
   * @param code код программы, который будет запущен в браузере
   */
  execute(code: string): void;
  /**
   * Создаёт снимок экрана игры и помещает его в каталог /screenshots/ip_port.
   * @remarks
   * Скриншоты доступны в CEF по адресу http://screenshots/name.png.
   * @param name наименование снимка
   * @param type формат изображения
...
interface BlipMp {
  ...
  // TODO: узнать, что делает данная функция.
  ...
  destroy(): void;
  /**
   * Проверяет, существует метка на карте или нет.
   * @return результат проверки (true - метка существует).
   */
  doesExist(): boolean;
  // TODO: узнать, что делает данная функция.
  endTextCommandSetName(): void;
  /**
   * Получить значение прозрачности метки.
   * @return значение альфа-канала цвета (прозрачность).
   */
  getAlpha(): number;
  /**
   * Получить цвет метки.
   * @remarks
   * Варианты цветов метки можно просмотреть [здесь](https://wiki.rage.mp/index.php?title=Blips#Blip_colors).
   * @return номер цвета.
   */
  getColour(): number;
  /**
   * Получает координаты метки на карте.
   * @return координаты.
   */
  getCoords(): Vector3Mp;
  // TODO: узнать, что делает данная функция.
  ...
  /**
   * Получает значение о том, к чему прикреплена метка.
   * @return тип объекта:
   * - 1: транспорт
   * - 2: ped
   * - 3: объект
  ...
  /**
   * Получает номер модели метки.
   * @remarks
   * Список моделей метки можно просмотреть [здесь](https://wiki.rage.mp/index.php?title=Blips#Blip_model).
   * @return номер модели.
   */
  getSprite(): number;
  ...
  hideNumberOn(): void;
  /**
   * Получает статус мигания метки.
   * @return состояние метки (true - метка мигает).
   */
  isFlashing(): boolean;
  /**
   * Проверяет, установлена метка в качестве создателя миссии или нет.
   * @return статус метки.
   */
  isMissionCreator(): boolean;
  /**
   * Проверяет, отображается метка на миникарте или нет.
   * @return состояние отображения (true - метка видна на миникарте).
   */
  isOnMinimap(): boolean;
  /**
   * Проверяет, является ли метка видимой только на короткой дистанции или нет.
   * @return результат проверки (true - метка видна только в области мини-карты).
   */
  isShortRange(): boolean;
  pulse(): void;
  /**
   * Устанавливает прозрачность метки.
   * @param alpha альфа-канал цвета (0-255)
   */
  setAlpha(alpha: number): void;
  /**
   * Устанавливает метке флаг союзника или противника.
   * @param toggle флаг метки (true - синяя, false - красная)
   */
  setAsFriendly(toggle: boolean): void;
  /**
   * Устанавливает метку в качестве создателя миссии.
   * @param toggle флаг активации
   */
  setAsMissionCreator(toggle: boolean): void;
  /**
   * Устанавливает метке модификатор короткого диапазона видимости (исчезает, когда метка не находится в диапазоне мини-карты).
   * @param toggle флаг активации
   */
  setAsShortRange(toggle: boolean): void;
  setBright(toggle: boolean): void;
  /**
   * Устанавливает категорию метки.
   * @param index индекс категории:
   * - 1 = отсутствует текст на метке или расстоянии
   * - 2 = текст на метке
   * - 3 = отсутствует текст, только расстояние
   * - 7 = имена других игроков (расстояний)
   * - 10 = собственность
   * - 11 = занятая собственность
   * - \> 11 = отсутствует текст на метке или расстоянии
   */
  setCategory(index: number): void;
  /**
   * Устанавливает цвет метки.
   * @remarks
   * Список доступных цветов можно просмотреть [здесь](https://wiki.rage.mp/index.php?title=Blips#Blip_colors).
   * @param color цвет (по умолчанию жёлтый)
   */
  setColour(color: number): void;
  /**
   * Устанавливает координаты метки на карте.
   * @param position координаты
   */
  setCoords(position: Vector3Mp): void;
  /**
   * Устанавливает параметры видимости метки.
   * @param displayId id параметра:
   * - 8: отображается на карте и миникарте (нельзя выбрать на карте)
   * - 5 или 9: отображается только на миникарте
   * - 3 или 4: отображается только на карте (можно выбрать на карте)
   * - 2 или 6: отображается на карте и на миникарте (можно выбрать на карте)
   * - 0: никогда и нигде не появляется
   */
  setDisplay(displayId: number): void;
  setFade(opacity: number, duration: number): void;
  /**
   * Устанавливает мерцание метки.
   * @param toggle флаг активации мерцания
   */
  ...
  /**
   * Устанавливает socialClub имя игрока в качестве названия метки.
   * @param player игрок
   */
  setNameToPlayerName(player: PlayerMp): void;
  setPosition(posX: number, posY: number, posZ: number): void;
  /**
   * Устанавливает приоритет метки (порядок в легенде карты).
   * @param priority номер приоритета
   */
  setPriority(priority: number): void;
  /**
   * Устанавливает угол поворота метки.
   * @param rotation угол поворота
   */
  setRotation(rotation: number): void;
  /**
   * Устанавливает флаг отображения маршрута к метке.
   * @param enabled флаг отображения маршрута
   */
  setRoute(enabled: boolean): void;
  /**
   * Устанавливает цвет маршрута.
   * Список доступных цветов можно просмотреть [здесь](https://wiki.rage.mp/index.php?title=Blips#Blip_colors).
   */
  setRouteColour(colour: number): void;
  /**
   * Устанавливает относительный размер метки.
   * @param scale масштаб
   */
  ...
  /**
   * Отображает поворот игрока на метке, как в GTA:Online.
   * @param toggle флаг отображения поворота
   */
  setShowHeadingIndicator(toggle: boolean): void;
  /**
   * Устанавливает изображение метки и добавляет его на карту.
   * @remarks
   * Список доступных id меток можно просмотреть [здесь](http://gtaxscripting.blogspot.com/2016/05/gta-v-blips-id-and-image.html).
   * @param spriteId id изображения метки
   */
...
  readonly type: string;
  ...
  applyForceTo(forceType: number, x: number, y: number, z: number, xRot: number, yRot: number, zRot: number,
    boneIndex: number, isRel: boolean, p9: boolean, highForce: boolean, p11: boolean, p12: boolean): void;
  ...
  /**
   * Прикрепляет текущую сущность к кости второй сущности.
   * @remarks
   * boneIndex отличается от boneID, используйте GET_Player_BONE_INDEX для получения индекса из boneID. 
   * Используйте boneIndex для рикрепления к конкретной кости. Первая сущность будет прикреплена к центру второй сущности, если указанный boneIndex не соответствует индексам костей данного объекта.
   * @param entity вторая сущность, к которой прикрепляется текущая
   * @param boneIndex индекс кости (отличается от boneID)
   * @param xPos смещение относительно центра кости по оси x
   * @param yPos смещение относительно центра кости по оси y
   * @param zPos смещение относительно центра кости по оси z
   * @param xRot поворот по оси x
   * @param yRot поворот по оси y
   * @param zRot поворот по оси z
   * @param p8 true
   * @param useSoftPinning если установлено значение false, присоединённый объект не будет отсоединяться при исправлении
   * @param collision флаг коллизии между объектами (false отключает коллизию)
   * @param isPed false 
   * @param vertexIndex позиция вершины
   * @param fixedRot если false, игнорирует вектор сущности
   */
  ...
  /**
   * Привязывает текущую сущность к кости второй сущности с возможностью разрыва связи.
   * @param entity вторая сущность, к которой прикрепляется текущая
   * @param boneIndex1 индекс первой кости
   * @param boneIndex2 индекс второй кости
   * @param xPos1 смещение относительно центра первой кости по оси x
   * @param yPos1 смещение относительно центра первой кости по оси y
   * @param zPos1 смещение относительно центра первой кости по оси z
   * @param xPos2 смещение относительно центра второй кости по оси x
   * @param yPos2 смещение относительно центра второй кости по оси y
   * @param zPos2 смещение относительно центра второй кости по оси z
   * @param xRot поворот по оси x
   * @param yRot поворот по оси y
   * @param zRot поворот по оси z
   * @param breakForce сила, необходимая для разрыва связи
   * @param fixedRot если false, игнорирует вектор сущности
   * @param p14 всегда true
   * @param collision контролирует коллизию между двумя объектами (false отключает коллизию)
   * @param p16 флаг, позволяющий не телепортировать сущность для привязки к позиции boneIndex целевой сущности (true - сущность не будет телепортироваться в указанную кость)
   * @param p17 всегда 2
   */
  ...
  /**
   * Проверяет, существует сущность или нет.
  ...
  /**
   * Устанавливает заморозку для сущности.
   * @param toggle флаг заморозки (true - сущность неподвижна)
   */
  freezePosition(toggle: boolean): void;
  getAlpha(): number;
  /**
   * Получает значение времени воспроизведения анимации относительно общей длительности анимации.
   * @remarks
   * Значение увеличивается в диапазоне от 0 до 1 и возвращается к 0, когда достигает 1.
   * 
   * Пример:
   * - 0.0 - начало анимации
   * - 0.5 - середина анимации
   * - 1.0 - окончание анимации
   * @param animDict раздел анимации
   * @param animName название анимации
   * @return время относительного воспроизведения (от 0 до 1).
   */
  getAnimCurrentTime(animDict: string, animName: string): number;
  /**
   * Получает общее время воспроизведения анимации.
   * @param animDict раздел анимации
   * @param animName название анимации
   ...
   */
	...
	/**
	 * Получает handle объекта, с которой связана сущность.
	 * @return handle объекта
	 */
  getAttachedTo(): Handle;
  /**
   * Получает индекс кости по названию.
   * @remarks
   * Список названий можно просмотреть [здесь](pastebin.com/D7JMnX1g).
   * @param boneName название кости
   * @return индекс кости.
   */
  getBoneIndexByName(boneName: string): number;
  getCollisionNormalOfLastHitFor(): Vector3Mp;
  /**
   * Получает координаты текущей сущности.
   * @param alive состояние сущности (по умолчанию true)
   * @return координаты
   */
  getCoords(alive: boolean): Vector3Mp;
  /**
   * Получает направление сущности в виде вектора.
   * @remarks
   * Данная функция идентична функции Camera.getDirection.
   * @return направление
   */
  getForwardVector(): Vector3Mp;
  /**
   * Получает x компоненту вектора направления сущности
   * @return значение компоненты x
   */
  getForwardX(): number;
  /**
   * Получает y компоненту вектора направления сущности
   * @return значение компоненты y
   */
  getForwardY(): number;
  /**
   * Получает угол поворот объекта в градусах.
   * @return угол поворота.
   */
  getHeading(): number;
  /**
   * Получает текущее значение здоровья у сущности.
   * @remarks
   * Примеры диапазона значений у сущностей:
   * - Игрок [0 - 200]
   * - Ped [100 - 200]
   * - Транспорт [0 - 1000]
   * - Объект [0 - 1000]
  ...
  /**
   * Возвращает высоту относительно земли.
   * @return расстояние от земли до сущности
   */
  getHeightAboveGround(): number;
  getLastMaterialHitBy(): Hash;
  /**
   * Возвращает расстояние от уровня детализации текущей сущности
   * @return расстояние
   */
  getLodDist(): number;
  ...
  /**
   * Возвращает значение максимального здоровья сущности.
   * @remarks
   * Примеры:
   * - Игрок = 200
   * - Ped = 150
   * @return значение максимального здоровья (целое число)
   */
  getMaxHealth(): number;
  /**
   * Возвращает хэш модели данной сущности.
   * @return хэш модели
   */
  getModel(): Hash;
  ...
  /**
   * Возвращает все, что было передано.
   * @return хэш
   */
  getObjectIndexFromIndex(): Handle;
  /**
   * Преобразовывает абсолютные координаты в относительные текущей сущности.
   * @remarks
   * Пример:
   * posX = 50, x координата сущности = 40, возвращаемое значение x = 10 или -10
   * @param posX абсолютная координата x
   * @param posY абсолютная координата y
   * @param posZ абсолютная координата z
   * @return относительные координаты
   */
  getOffsetFromGivenWorldCoords(posX: number, posY: number, posZ: number): Vector3Mp;
  /**
   * Преобразовывает относительные координаты в абсолютные.
   * @remarks
   * Значения смещения указаны относительно сущности.
   * - x = влево/вправо
   * - y = вперед/назад
   * - z = вверх/вниз
   ...
   * @remarks
   * Получает более точное значение, чем функция Entity.getHeading. Это хорошо видно, например, во время тряски педа/игрока.
   * @return угол поворота (в градусах)
   */
  getPhysicsHeading(): number;
  /**
   * Отображает значение оси тангажа сущности.
   * @return значение оси тангажа [-180.0; 180.0]
   */
  getPitch(): number;
  ...
  /**
   * Отображает значение оси крена сущности.
   * @return значение оси крена [-180.0; 180.0]
   */
  getRoll(): number;
	getRotation(rotationOrder: number): Vector3Mp;
	/**
	 * Возвращает угловую скорость вращения.
	 * @return вектор угловой скорости
	 */
  getRotationVelocity(): Vector3Mp;
  getScript(script: Handle): Handle;
  /**
   * Возвращает скорость движения сущности.
   * @return скорости (метры в секунду)
   */
  getSpeed(): number;
  /**
   * Возвращает вектор скорости сущности.
   * @param relative используется для получения значения относительно автомобиля, например, чтобы определить, едет ли он задним ходом (-y) или нет (+y)
   * @return вектор скорости
   */
  getSpeedVector(relative: boolean): Vector3Mp;
  /**
   * Получает значение того, насколько объект погружён в воду (1.0 - целиком).
   * @return полнота погружения [0.0; 1.0]
   */
  ...
  /**
   * Получает вектор угловой скорости сущности.
   * @return вектор угловой скорости
   */
  getVelocity(): Vector3Mp;
  /**
   * Возвращает координаты кости сущности.
   * @param boneIndex индекс кости
   * @return координаты
   */
  getWorldPositionOfBone(boneIndex: number): Vector3Mp;
  hasAnimEventFired(actionHash: Hash): boolean;
  /**
   * Проверяет, было завершено воспроизведение анимации или нет.
   * @param animDict раздел анимации
   * @param animName название анимации
   * @param p2 всегда 3
   * @return результат проверки (true - завершено)
   */
  hasAnimFinished(animDict: string, animName: string, p2: number): boolean;
  /**
   * Проверяет, была сущность атакована другой сущностью или нет.
   * @param entity атакующий
   * @param p1 всегда true
   * @return результат проверки
   */
  hasBeenDamagedBy(entity: Handle, p1: boolean): boolean;
  /**
   * Проверяет, была сущность атакована каким-либо объектом или нет.
   * @return результат проверки
   */
  hasBeenDamagedByAnyObject(): boolean;
  /**
   * Проверяет, была сущность атакована каким-либо педом или нет.
   * @return результат проверки
   */
  hasBeenDamagedByAnyPed(): boolean;
  /**
   * Проверяет, была сущность атакована каким-либо транспортом или нет.
   * @return результат проверки
   */
  hasBeenDamagedByAnyVehicle(): boolean;
  hasClearLosTo(entity: Handle, traceType: number): boolean;
  /**
   * Проверяет, имеет сущность прямую видимость ко второй сущности или нет.
   * @remarks
   * Данная функция самая требовательная к ресурсам процессора; избегайте вызова во вложенных циклах.
   * @param entity сущность
   * @return результат проверки
   */
  hasClearLosToInFront(entity: Handle): boolean;
  /**
   * Проверяет, касается корпус транспорта какого-либо объекта или нет.
   * @remarks
   * Для транспорта колёса могут касаться земли, но функция будет возвращать значение false. Функция вернёт true только когда корпус транспорта будет касаться земли.
   * @return результат проверки.
   */
  hasCollidedWithAnything(): boolean;
  hasCollisionLoadedAround(): boolean;
  /**
   * Проверяет, является сущность заданием или нет.
   */
  isAMission(): boolean;
  /**
   * Проверяет, является сущность объектом или нет.
   */
  isAnObject(): boolean;
  /**
   * Проверяет, является сущность педом или нет.
   */
	isAPed(): boolean;
	/**
	 * Проверяет, находится ли сущность внутри параллелепипеда, определяемый размерами `xSize`, `ySize` и `zSize`, внутри сущности `entity`.
	 * @param entity сущность-объект
	 * @param xSize размер области x
	 * @param ySize размер области y
	 * @param zSize размер области z
	 * @param p4 почти всегда false
	 * @param p5 почти всегда true
	 * @param p6 почти всегда false
	 */
  isAt(entity: Handle, xSize: number, ySize: number, zSize: number, p4: boolean, p5: boolean, p6: number): boolean;
	/**
	 * Проверяет, находится сущность в пределах (`xSize`, `ySize`, `zSize`) расстояния от (`xPos`, `yPos`, `zPos`).
	 * @param xPos позиция x
	 * @param yPos позиция y
	 * @param zPos позиция z
	 * @param xSize размер области x
	 * @param ySize размер области y
	 * @param zSize размер области z
	 * @param p6 почти всегда false
	 * @param p7 почти всегда true
	 * @param p8 почти всегда false
	 */
	...
	/**
	 * Проверяет, прикреплена сущность к чему-либо или нет
	 */
  isAttached(): boolean;
  /**
   * Проверяет, прикреплена сущность к указанному объекту или сущности или нет.
   * @param to сущность или объект
   */
	isAttachedTo(to: Handle): boolean;
	/**
	 * Проверяет, прикреплена сущность к какому-либо объекту или нет
	 */
	isAttachedToAnyObject(): boolean;
	/**
	 * Проверяет, прикреплена сущность к какому-либо педу или нет
	 */
	isAttachedToAnyPed(): boolean;
	/**
	 * Проверяет, прикреплена сущность к какому-либо транспорту или нет
	 */
  isAttachedToAnyVehicle(): boolean;
  /**
   * Проверяет, является сущность транспортом или нет.
   */
	isAVehicle(): boolean;
	/**
	 * Проверяет, отключена у сущности коллизия или нет.
	 */
  isCollisonDisabled(): boolean;
	isDead(): boolean;
	/**
	 * Проверяет, находится сущность в воздухе или нет.
	 */
	isInAir(): boolean;
	/**
	 * Создаёт сферический конус в исходной точке, который продолжается до поверхности с указанным углом. Затем возвращает истину, если объект находится внутри сферического конуса.
	 * @param originX исходная координата x
	 * @param originY исходная координата y
	 * @param originZ исходная координата z
	 * @param edgeX граница основания конуса по x
	 * @param edgeY граница основания конуса по y
	 * @param edgeZ граница основания конуса по z
	 * @param angle угол конуса (в градусах)
	 * @param p7 = false
	 * @param p8 = true
	 * @param p9 = false
	 */
  ...
  isInArea(x1: number, y1: number, z1: number, x2: number, y2: number, z2: number, p6: boolean, p7: boolean, p8: any): boolean;
	/**
	 * Проверяет, находится сущность в воде или нет.
	 */
	isInWater(): boolean;
  isInZone(zone: string): boolean;
	isOccluded(): boolean;
	/**
	 * Проверяет, находится сущность между минимальным и максимальным значениями 2d координат экрана.
	 * @remarks
	 * Функция вернёт истину, даже если объект находится за стеной, но вы смотрите на него.
	 */
	isOnScreen(): boolean;
	/**
	 * Проверяет, воспроизводится указанная анимация или нет.
	 * @param animDict раздел анимаций
	 * @param animName название анимации
	 * @param p2 всегда 3
	 */
  ...
  playSynchronizedAnim(syncedScene: number, animation: string, propName: string, p3: number, p4: number, p5: any,
		p6: number): boolean;
	/**
	 * Обновляет прикрепления сущности.
	 */
	processAttachments(): void;
	/**
	 * Сбрасывает значение альфа-канала.
	 */
	resetAlpha(): void;
	/**
	 * Устанавливает значение альфа-канала для сущности.
	 * @param alphaLevel от 0 до 255, но изменения заметны с шагом в 20% (после каждого 51 значения)
	 */
  ...
  setAnimCurrentTime(animDict: string, animName: string, time: number): void;
	 ...
	 * @param animDict раздел анимаций
	 * @param animName название анимации
	 * @param speedMultiplier коэффициент скорости
	 */
	setAnimSpeed(animDict: string, animName: string, speedMultiplier: number): void;
	/**
	 * Делает указанную сущность постоянной.
	 * @remarks
	 * Постоянные сущности не будут автоматически удалены движком.
	 */
  ...
  setCanBeDamaged(toggle: boolean): void;
	...
	/**
	 * Устанавливает значение того, может сущность быть под наведением не находясь в зоне прямой видимости или нет.
	 * @param toggle флаг
	 */
	setCanBeTargetedWithoutLos(toggle: boolean): void;
	/**
	 * Устанавливает значение коллизии сущности.
	 * @param toggle флаг коллизии (false отключает коллизию и сущность будет проваливаться сквозь текстуры)
	 * @param keepPhysics
	 */
	setCollision(toggle: boolean, keepPhysics: boolean): void;
	/**
	 * Перемещает сущность в указанные координаты.
	 * @param xPos координата x
	 * @param yPos координата y
	 * @param zPos координата z
	 * @param xAxis инвертирует ось x
	 * @param yAxis инвертирует ось y
	 * @param zAxis инвертирует ось z
	 * @param clearArea = false
	 */
  setCoords(xPos: number, yPos: number, zPos: number, xAxis: boolean, yAxis: boolean, zAxis: boolean, clearArea: boolean): void;
	/**
	 * Делает то же самое, что и Entity.setCoords.
	 */
	setCoords2(xPos: number, yPos: number, zPos: number, xAxis: number, yAxis: number, zAxis: number, clearArea: boolean): void;
	/**
	 * Перемещает сущность в указанные координаты без смещения.
	 * @param xPos координата x
	 * @param yPos координата y
	 * @param zPos координата z
	 * @param xAxis инвертирует ось x
  ...
  setDynamic(toggle: boolean): void;
	 ...
	 * @param heading угол поворота в градусах [0.0; 360.0]
	 */
	setHeading(heading: number): void;
	/**
	 * Устанавливает указанное здоровье сущности.
	 * @remarks
	 * - __Игрок__: по какой-то причине здоровье рассчитывается как 100 + текущее здоровье игрока. Установка значения = 100 означает, что у игрока будет 0 очков здоровья, но он не умрёт, значение < 100 убьёт игрока.
	 * - __Транспорт__: установка здоровья транспорта, похоже, не приводит к какому-либо результату. Транспорт не будет уничтожен при отметке 0 даже после любого повреждения.
	 ...
	 * @remarks
	 * Сущность не получает никаких повреждений, её не отталкивает взрывной волной и на ней не работает анимация шокера.
	 * @param toggle флаг
	 */
  setInvincible(toggle: boolean): void;
	setIsTargetPriority(p0: boolean, p1: number): void;
	/**
	 * Включает или выключает огни у объекта.
	 */
  setLights(toggle: boolean): void;
	setLoadCollisionFlag(toggle: boolean): void;
	/**
	 * Устанавливает максимальное расстояние, на котором будет прогружаться объект.
	 * @param value расстояние
	 */
	setLodDist(value: number): void;
	/**
	 * Устанавливает максимальное значение здоровья объекта.
	 * @param value значение здоровья
	 */
	setMaxHealth(value: number): void;
	/**
	 * Устанавливает ограничитель скорости для объекта.
	 * @param speed скорость (метры в секунду)
	 */
	setMaxSpeed(speed: number): void;
	/**
	 * Устанавливает размытие объекта при движении на экране.
	 */
	setMotionBlur(toggle: boolean): void;
	/**
	 * Устанавливает значение коллизии между двумя объектами.
	 * @param entity второй объект
	 * @param collision значение коллизии
	 */
  ...
  setOnlyDamagedByPlayer(toggle: boolean): void;
	...
	/**
	 * Устанавливает сопротивление к указанным типам урона.
	 * @param bulletProof восприимчивость к ранениям огнестрельным оружием
	 * @param fireProof восприимчивость к огню
	 * @param explosionProof восприимчивость ко взрывам
	 * @param collisionProof восприимчивость к столкновениям
	 * @param meleeProof восприимчивость к ранениям в рукопашном бою
	 * @param p5 = true
	 * @param p6 = true
	 * @param drownProof устойчивость к плаванию под водой
	 */
  ...
  setRecordsCollisions(toggle: boolean): void;
	...
	/**
	 * Установить угол тангажа, крена и рысканья.
	 * @param pitch угол тангажа
	 * @param roll угол крена
	 * @param yaw угол рысканья
	 * @param rotationOrder порядок выполнения поворотов [0; 5]
	 * @param p4 = true
	 */
  ...
  setTrafficlightOverride(state: number): void;
	/**
	 * Устанавливает значение переменной объекта.
	 * @param key название переменной
	 * @param value значение переменной
	 */
	setVariable(key: string, value: any): any;
	/**
	 * Установить угловую скорость объекта.
	 * @param x угловая скорость по оси x
	 * @param y угловая скорость по оси y
	 * @param z угловая скорость по оси z
	 */
	setVelocity(x: number, y: number, z: number): void;
	/**
	 * Устанавливает видимость объекта.
	 * @param флаг видимости (false - объект невидимый)
	 * @param p1 = false
	 */
	setVisible(toggle: boolean, p1: boolean): void;
	/**
	 * Останавливает воспроизведение указанной анимации
	 * @param animation название анимации
	 * @param animGroup раздел анимаций
	 * @param p2
	 */
...
interface GameEntityMp {
  ...
  createModelHideExcludingScriptObjects(x: number, y: number, z: number, radius: number, model: Hash,
    p5: boolean): void;
  /**
   * Изменяет текущую модель на новую (работает только с объектами!).
   * @remarks
   * Другие игроки не увидят изменений.
   * @param x координата x, относительно которой будет действовать данная функция
   * @param y координата y, относительно которой будет действовать данная функция
   * @param z координата z, относительно которой будет действовать данная функция
   * @param radius радиус, в котором будет действовать функция
   * @param originalModel установленная текущая модель
   * @param newModel новая модель, на которую будет заменена старая модель
   * @param p6
   */
